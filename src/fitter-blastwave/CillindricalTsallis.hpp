...
/*
From arxiv:2309.06128v3
*/
namespace Cillindrical_3 {
double Integrand(double *x, double *par) {
    double xi = x[0];
    double phi = x[1];
...
double Function(double *x, double *par) {
    ...
    const double twopi = 1 / (2 * M_PI);

    double resint = integration(xi_min, xi_max, phi_min, phi_max);
    double term2 = TMath::Power(1 + (qpp - 1) * mT / Tts, qpp / (1 - qpp));

    return mT * A * (resint + (term2 + resint) * TMath::Exp(-tftau));
}
}  // namespace Cillindrical_3
