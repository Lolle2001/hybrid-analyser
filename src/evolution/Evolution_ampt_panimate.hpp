   ...
   public:
            ...
            while (File >> TempBlock.Time) {
                if (TempBlock.Time == -10) {
                    ...
                    for (int i = 0; i < TempBlock.NumberOfParticles; ++i) {
                        ...
                        if (!finalft->FreezeOutMap[TempPair]) {
    ...
    void PrintPartonEnergyDensity(std::string Directory) {
        for (int i = 0; i < PartonEnergyDensity.size(); ++i) {
            std::ofstream file(Directory + "/PartonEnergyDensity_" + std::to_string(i) + ".dat");
    ...
    void Write(int NRun, std::string Directory) {
        std::ofstream File;
        std::string FileName = Directory + std::to_string(NRun) + "/panimate_summary.dat";
        File.open((FileName).c_str(), std::ios::out);
        if (File.is_open()) {
            for (int i = 0; i < VectorOfParseBlocks.size(); ++i) {
                File << VectorOfParseBlocks[i].Time << " " << VectorOfParseBlocks[i].NumberOfParticles << " " << VectorOfParseBlocks[i].CountMap.size() << std::endl;
                for (auto Entry : VectorOfParseBlocks[i].CountMap) {
